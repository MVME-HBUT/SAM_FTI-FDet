...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., drop=0.,
                 drop_path=0., downsample=None, use_checkpoint=True,
                 local_conv_size=3,
                 activation=nn.GELU,
                 out_dim=None,
                 ):
...
class TinyViTSAM(nn.Module):
    def __init__(self, img_size=1024, in_chans=3, num_classes=15,
    ...
    def forward_features(self, x):
        ...
        x=x.permute(0, 3, 1, 2)
        x=self.neck(x)
        # interm_embeddings[0] = torch.cat([interm_embeddings[0], interm_embeddings[0]], dim=3)
        # for channal in interm_embeddings:
        #     print(channal.shape)
        # assert False
